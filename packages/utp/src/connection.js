...
function Connection (utp, port, address, handle, halfOpen) {
  ...
  this._index = -1
  this._utp = utp
  this._handle = handle || Buffer.alloc(binding.sizeof_utp_rn_connection_t)
  ...
  binding.utp_rn_connection_init(this._handle, this, this._buffer,
    this._onread,
    this._ondrain,
    this._onend,
    this._onerror,
...
Connection.prototype._write = function (data, enc, cb) {
  ...
  if (!this._connected || !binding.utp_rn_connection_write(this._handle, data)) {
    this._callback = cb
...
Connection.prototype._writev = function (datas, cb) {
  ...
  if (!binding.utp_rn_connection_writev(this._handle, bufs)) {
    this._callback = cb
    this._writing = bufs
    return
  }
...
Connection.prototype._connect = function (port, ip) {
  ...
  binding.utp_rn_connect(this._utp._handle, this._handle, port, ip)
}
...
Connection.prototype.destroy = function (err) {
  ...
  binding.utp_rn_connection_close(this._handle)
}
...
Connection.prototype._shutdown = function () {
  if (this.destroyed) return
  binding.utp_rn_connection_shutdown(this._handle)
  this._destroyMaybe()
}

function onbindingclose (self) {
  binding.utp_rn_connection_on_close(self._handle)
}
