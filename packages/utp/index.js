...
function UTP (opts) {
  ...
  this._handle = Buffer.alloc(binding.sizeof_utp_rn_t)
  this._nextConnection = Buffer.alloc(binding.sizeof_utp_rn_connection_t)
  this._address = null
  this._inited = false
  this._refed = true
  this._closing = false
  ...
  this._acceptConnections = new Uint32Array(this._handle.buffer, this._handle.byteOffset + binding.offsetof_utp_rn_t_accept_connections, 1)
...
UTP.prototype._init = function () {
  ...
  binding.utp_rn_init(this._handle, this,
...
UTP.prototype.ref = function () {
  if (this._inited) binding.utp_rn_ref(this._handle)
  this._refed = true
}

UTP.prototype.unref = function () {
  if (this._inited) binding.utp_rn_unref(this._handle)
  this._refed = false
}
...
UTP.prototype.address = function () {
  ...
  return {
    ...
    family: 'IPv4',
    port: binding.utp_rn_local_port(this._handle)
  }
}
...
UTP.prototype.send = function (buf, offset, len, port, host, cb) {
  ...
  if (!send) {
    ...
    binding.utp_rn_send_request_init(send._handle, send)
  }
  ...
  binding.utp_rn_send(this._handle, send._handle, send._buffer, offset, len, port, host)
}

UTP.prototype._resolveAndSend = function (buf, offset, len, port, host, cb) {
  const self = this

  binding.lookup(host, onlookup)
...
UTP.prototype.bind = function (port, ip, onlistening) {
  ...
  try {
    binding.utp_rn_bind(this._handle, port, ip)
  } catch (err) {
    this._address = null
...
UTP.prototype._resolveAndBind = function (port, host) {
  ...
  binding.lookup(host, function (err, ip) {
...
UTP.prototype._onclose = function () {
  binding.utp_rn_destroy(this._handle, this._sent.map(toHandle))
  this._handle = null
  this.emit('close')
}

function SendRequest () {
  this._handle = Buffer.alloc(binding.sizeof_utp_rn_send_request_t)
  this._buffer = null
  this._callback = null
  this._index = null
}
